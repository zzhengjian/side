...
LocatorBuilders.prototype.buildAll = function(el) {
  let locator
  let locators = []
  for (let order of LocatorBuilders.order) {
    let finderName = order
    try {
...
LocatorBuilders.prototype.findElement = function(loc) {
  try {
    ...
    let elements =findElement({ [locator.type]: locator.string }, this.window.document)
    return elements[0]
  } catch (error) {
...
LocatorBuilders.prototype.findElements = function(loc, root) {
  try {
    const locator = parse_locator(loc, true)
    return findElement({ [locator.type]: locator.string }, root)
  } catch (error) {
    //this.log.debug("findElement failed: " + error + ", locator=" + locator);
    return null
  }
